...
void Graph::update()
{
    ...
    if( keypressed() )
    {
        ...
        toutesLesComposanteFortementConnexe();
    }
...
void Graph::toutesLesComposanteFortementConnexe()
{
...
//init a 0 des deux tabs
    ...
    std::cout << "CPT :"<<cpt;
    std::ofstream fichier( "test.txt", std::ios::out | std::ios::trunc ); // ouverture en écriture avec effacement du fichier ouvert
    if( fichier )
    {
        for ( int i = 0; i < m_vertices.size(); i++ )
        {
...
std::vector<std::vector<int>> Graph::transpose()
{
    ...
    T.resize( m_vertices.size(), std::vector<int>( m_vertices.size(), 0 ) );//resize double vector
    for ( int i = 0; i < m_vertices.size(); i++ )
    {
        for ( int j = 0; j < m_vertices.size(); j++ )
        {
