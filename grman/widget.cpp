...
void Widget::update_interact()
{
    ...
    /// On interagit en 1er avec les �l�ments ajout�s en dernier
...
/// Gestion des affichages, 1�re passe pour fixer les cadres
/// (N�cessaire pour les liens : affich�s au fond donc en 1er mais s'appuyant sur les positions des autres qui sont affich�s apr�s)
...
void Widget::update_draw()
{
    ...
    /// Propagation de l'update aux elements enfants
    /// On affiche en dernier (devant les autres) les �l�ments ajout�s en dernier
    for (auto &e : m_children)
        e->update_draw();
...
{
    /// Calculer absolute frame � partir de relative et absolute parent ( si parent, sinon page_frame )
    ...
    // Petits soucis de plantages avec les sub_bitmaps hors �cran...
    ...
    // Malheureusement ceci ne marche pas ... (� creuser)
...
//                masaisie[m_i] = touche1;
                ...
                set_message( str );
               //textprintf_ex( m_view, font, 0, 0, m_color, -1, "salut!");
                //textprintf_ex( m_view, font, 0, 0, m_color, -1, m_message.c_str() );
                i++;
                // textprintf(screen,font,X_SAISIE+8*i,Y_SAISIE,makecol(237,235,229),"_");
            }
...
void WidgetTimer::interact_focus()
{
    if ( mouse_click )
        m_value = !m_value;
}
...
void WidgetEdge::draw()
{
    ...
    // Vecteur de centre � centre
    Coords vec_dir = p[1] - p[0];

    // D�termination des intersections avec les frames des 2 widgets li�s
    p[0] = m_attach[0]->get_abs_frame().intersect(vec_dir);
    p[1] = m_attach[1]->get_abs_frame().intersect(-vec_dir);

    // Dessin du lien cadre � cadre
    thick_line(page, p[0].x, p[0].y, p[1].x, p[1].y, m_thickness, m_color);

    // Calcul du nouveau vecteur cadre � cadre
    vec_dir = p[1] - p[0];

    // Point d'ancrage pour les �ventuels enfants ( Widgets attach�s � l'ar�te )
    ...
    /// La suite concerne les �l�ments de d�corations, fl�ches/extr�mit�s
    /// Pour chaque �l�ment de d�coration du lien...
